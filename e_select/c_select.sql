...
*/
select * from `members`;
-- 데이터 삽입 순서대로 정렬 (auto_increment PK값에 따라 정렬)
...
select * from `members` order BY grade desc, points desc;

/*
2. limit
: 출력하는 개수를 제한 (반환하는 행의 수를 제한)
limit 행수 (offset 시작행)

cf) 첫 번째 행이 기본값 0
*/
select * from `members` limit 5 offset 2;
-- 시작번호 2는 3번째 데이터 부터 (index 번호 체계와 동일)

select * from `members` order BY grade DESC LIMIT 5;
...
select grade, count(*) as member_count from `members` group by grade;

# 지역별 평균 포인트 계산
select area_code, avg(points) as member_point from `members` group by area_code;
