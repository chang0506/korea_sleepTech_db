...
! 2. 외부 조인(Outer Join): 한쪽 테이블에 일치하지 않아도 결과로 포함 (==합집합)
  ...
  3. 상호 조인(Cross Join): 모든 조합을 반환 (테이블 1 * 테이블 2)
...
/*
  == 1. 내부 조인(INNER JOIN) ==
  : 두개 이상의 테이블에서 특정 열(기준 열)의 값이 일치하는 행만 가져오는 조인
...
select M.name, sum(P.amount) total_purchases
# inner join은 가장 기본 조인이기 때문에 inner 키워드 생략가능
from members M inner join purchases P on M.member_id = P.member_id
group by M.name
