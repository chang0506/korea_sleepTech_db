...
# : 정규화의 두 번쩨 단계
# - 1NF(제1정규화)를 만족하면서 '모든 비기본 속성'이 기본키에 완전히 종속되는 상태
# - 완전 함수적 종속 상태
...
# cf) 완전 함수적 종속
#     : 속성이 기본키 전체에 종속되어 있으며 기본키 일부분 만으로는 결정할 수 없는 경우
...
create table `employees` (
  employee_id VARCHAR(10) PRIMARY KEY, -- 기본키
...
VALUES
  ('HR', 'seoul', 's1'),
  ('SALES', 'busan', 's2'); -- location과 supervisor_id 속성이 기본키에 완전 종속
